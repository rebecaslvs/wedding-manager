  ...
  <div class="admin">
    <div class="admin-container">
      ...
      <div v-if="activeTab === 'Presentes'" class="tab-content">
          ...
          <form @submit.prevent="addNewGift" class="form">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input v-model="newGift.category" type="text" id="newGiftCategory" placeholder="Ex: Cama e Banho" required />
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
              <div class="form-group">
                <label>Imagem:</label>
                <input type="file" accept="image/*" @change="onNewGiftImageChange" />
                <div v-if="newGift.image" class="image-preview" style="margin-top:8px;">
                  <img :src="newGift.image" alt="Prévia da imagem" style="max-width:120px;max-height:120px;border-radius:8px;" />
                </div>
              </div>
            </div>
            ...
            <table>
              ...
              <tbody>
                <tr v-for="gift in store.gifts" :key="gift.id">
                  ...
                  <td>
                    <img v-if="gift.image" :src="gift.image" alt="Imagem do presente" style="max-width:60px;max-height:60px;border-radius:6px;" />
                    <span v-else>—</span>
                  </td>
                  <td>
                  ...
                  <td>
                    <button @click="openEditGift(gift)" class="btn-edit">Editar</button>
        ...
        <!-- Modal de edição de presente -->
        <div v-if="showGiftModal" class="modal-overlay">
          <div class="modal-content" @click.stop>
            <h3>{{ editingGift && editingGift.id ? 'Editar Presente' : 'Adicionar Presente' }}</h3>
            <form @submit.prevent="saveEditGift" class="form">
              <div class="form-group">
                <label>Título</label>
              ...
              <div class="form-group">
                <label>Imagem</label>
                <input type="file" accept="image/*" @change="onEditGiftImageChange" />
                <div v-if="giftForm.image" class="image-preview" style="margin-top:8px;">
                  <img :src="giftForm.image" alt="Prévia da imagem" style="max-width:120px;max-height:120px;border-radius:8px;" />
                </div>
              </div>
              <div class="modal-actions">
                <button type="submit" class="btn btn-primary">Salvar</button>
                <button type="button" class="btn btn-cancel" @click="closeGiftModal">Cancelar</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <tbody>
              <tr v-for="purchase in store.purchases" :key="purchase.id">
                ...
                <td style="display:flex;gap:0.5rem;align-items:center;">
                  ...
                  <button v-if="purchase.status==='pending'" @click="markPurchaseCompleted(purchase.id)" class="btn-edit" style="background:#4caf50;">Marcar como Concluída</button>
                  <button v-else @click="markPurchasePending(purchase.id)" class="btn-edit" style="background:#ffc107;color:#333;">Marcar como Pendente</button>
                </td>
              </tr>
            </tbody>
          </table>
...
function onNewGiftImageChange(e) {
  const file = e.target.files[0]
  if (!file) return
  const reader = new FileReader()
  reader.onload = ev => {
    newGift.value.image = ev.target.result
  }
...
function openEditGift(gift) {
  if (gift) {
    ...
    giftForm.value = { id: '', title: '', category: '', description: '', value: 0, image: '' }
  }
...
function onEditGiftImageChange(e) {
  const file = e.target.files[0]
  if (!file) return
  const reader = new FileReader()
  reader.onload = ev => {
    giftForm.value.image = ev.target.result
  }
  reader.readAsDataURL(file)
}
...
function markPurchaseCompleted(id) {
  store.updatePurchase(id, { status: 'completed' })
  store.loadData()
}
function markPurchasePending(id) {
  store.updatePurchase(id, { status: 'pending' })
  store.loadData()
}
