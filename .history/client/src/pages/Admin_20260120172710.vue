...
<div v-if="editingGuest" class="modal-overlay" @click="editingGuest = null">
  <div class="modal-content" @click.stop>
    <h3>Editar Convidado</h3>
    <form @submit.prevent="saveEditedGuest" class="form">
      <div class="form-row">
        <div class="form-group">
          <label>Nome:</label>
          <input v-model="editingGuest.name" type="text" required />
        </div>
        <div class="form-group">
          <label>WhatsApp:</label>
          <input v-model="editingGuest.whatsapp" type="tel" required />
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Limite de Adultos:</label>
          <input v-model.number="editingGuest.adultsCount" type="number" min="0" required />
        </div>
        <div class="form-group">
          <label>Limite de Crianças:</label>
          <input v-model.number="editingGuest.childrenCount" type="number" min="0" required />
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Nomes dos Adultos:</label>
          <ul>
            <li v-for="(name, idx) in editingGuest.adultNames || []" :key="'adult-edit-' + idx">
              <input v-model="editingGuest.adultNames[idx]" type="text" />
              <button type="button" @click="removeAdultName(idx)">Excluir</button>
            </li>
          </ul>
          <button type="button" @click="addAdultName">+ Adicionar Adulto</button>
        </div>
        <div class="form-group">
          <label>Nomes das Crianças:</label>
          <ul>
            <li v-for="(name, idx) in editingGuest.childNames || []" :key="'child-edit-' + idx">
              <input v-model="editingGuest.childNames[idx]" type="text" />
              <button type="button" @click="removeChildName(idx)">Excluir</button>
            </li>
          </ul>
          <button type="button" @click="addChildName">+ Adicionar Criança</button>
        </div>
      </div>
      <div class="modal-actions">
        <button type="submit" class="btn btn-primary">Salvar</button>
        <button type="button" class="btn btn-cancel" @click="editingGuest = null">Cancelar</button>
      </div>
    </form>
  </div>
</div>
// Funções para manipular acompanhantes no modal de edição
function addAdultName() {
  if (!editingGuest.value.adultNames) editingGuest.value.adultNames = []
  editingGuest.value.adultNames.push('')
}
function removeAdultName(idx: number) {
  editingGuest.value.adultNames.splice(idx, 1)
}
function addChildName() {
  if (!editingGuest.value.childNames) editingGuest.value.childNames = []
  editingGuest.value.childNames.push('')
}
function removeChildName(idx: number) {
  editingGuest.value.childNames.splice(idx, 1)
}

// Salvar edição do convidado
function saveEditedGuest() {
  store.updateGuest(editingGuest.value.id, {
    name: editingGuest.value.name,
    whatsapp: editingGuest.value.whatsapp,
    adultsCount: editingGuest.value.adultsCount,
    childrenCount: editingGuest.value.childrenCount,
    adultNames: editingGuest.value.adultNames,
    childNames: editingGuest.value.childNames
  })
...
// Estado para convidado em edição
const editingGuest = ref(null)

// Função para iniciar edição de convidado
function editGuest(guest: any) {
  editingGuest.value = { ...guest }
  // Aqui você pode abrir um modal ou exibir um formulário de edição
  alert('Função de edição de convidado em desenvolvimento!')
}
