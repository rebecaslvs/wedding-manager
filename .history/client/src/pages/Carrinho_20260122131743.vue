<template>
  <div class="carrinho">
      ...
      <div class="modal" @click.stop>
        ...
        <div class="pix-content">
          <div class="pix-info">
            ...
            <p><strong>Itens:</strong> {{ store.cartItemsCount }}</p>
          </div>

          <div class="pix-dados-container">
            <h3>Dados para pagamento via Pix</h3>
            <div class="pix-dados-list">
              <p><strong>Chave Pix:</strong> {{ store.weddingInfo.pixKey || 'Não configurada' }}</p>
              <p><strong>Valor:</strong> R$ {{ formatPrice(store.cartTotal) }}</p>
              <p><strong>Nome:</strong> Rebeca Silva Alves</p>
              <p><strong>Banco:</strong> Nubank</p>
            </div>
            <div class="pix-orientacao">
              <p>Após realizar o pagamento, marque abaixo se concluiu e, se preferir, envie o comprovante para o WhatsApp <a href="https://wa.me/5577988289651" target="_blank">(77) 98828-9651</a>.</p>
            </div>
            <div class="pix-confirmacao">
              <label style="display:flex;align-items:center;gap:0.5rem;">
                <input type="checkbox" v-model="pixPago" />
                Concluí o pagamento
              </label>
            </div>
            <div class="pix-botoes" style="margin-top:1.5rem;display:flex;gap:1rem;">
              <button :disabled="!pixPago" @click="confirmPixPayment" class="btn btn-primary">Confirmar Pagamento</button>
    ...
    <!-- Modal para Links de Pagamento ou WhatsApp -->
    <div v-if="showLinksModal" class="modal-overlay" @click="closeLinksModal">
      <div class="modal" @click.stop>
        <button class="modal-close" @click="closeLinksModal">✕</button>
        <!-- Se for compra via link e mais de um item/unidade, mostrar WhatsApp -->
        <template v-if="shouldShowWhatsappModal">
          <h2>Finalizar pelo WhatsApp</h2>
          <div class="links-content">
            <p>Como há mais de um item ou unidade no carrinho, o pagamento será finalizado pelo WhatsApp. Escolha com quem deseja falar:</p>
            <div class="modal-actions" style="justify-content: center; gap: 1.5rem; margin-top: 2rem;">
              <a :href="whatsappLinkRebeca" target="_blank" class="btn btn-primary">Rebeca<br><small>(77) 98828-9651</small></a>
              <a :href="whatsappLinkMarcos" target="_blank" class="btn btn-primary">Marcos<br><small>(77) 98809-7836</small></a>
            </div>
          </div>
        </template>
        <template v-else>
          <h2>Escolha o Link de Pagamento</h2>
          <div class="links-content">
            <div v-if="hasAvailableLinks" class="links-list">
              <p class="links-subtitle">Selecione um link para cada item:</p>
              <div v-for="(item, index) in cartItems" :key="index" class="link-item">
                <p class="item-name">{{ item.title }} ({{ item.quantity }}x)</p>
                <div class="link-options">
                  <button
                    v-for="(link, linkIndex) in getAvailableLinksForItem(index)"
                    :key="linkIndex"
                    @click="selectPaymentLink(index, link)"
                    :class="['btn-link', { selected: selectedLinks[index] === link }]"
                  >
                    Link {{ linkIndex + 1 }}
                  </button>
                  <!-- Ao invés de copiar, mostra botões WhatsApp -->
                  <button
                    v-for="(link, linkIndex) in getAvailableLinksForItem(index)"
                    :key="'whatsapp-' + linkIndex"
            ...
            <div v-else class="links-error">
              <p>⚠️ Nenhum link de pagamento disponível para este presente.</p>
            </div>
            <div class="modal-actions">
              <button
                v-if="hasAvailableLinks"
                @click="confirmLinkPayment"
...
// Computed para saber se deve mostrar o modal do WhatsApp
const shouldShowWhatsappModal = computed(() => {
  if (checkoutData.value.paymentMethod !== 'link') return false;
  // Mais de um item diferente OU alguma unidade > 1
  if (cartItems.value.length > 1) return true;
  if (cartItems.value.length === 1 && cartItems.value[0].quantity > 1) return true;
  return false;
});

const whatsappMsg = computed(() => {
  // Mensagem padrão para o WhatsApp
  const nome = checkoutData.value.name || '';
  const tel = checkoutData.value.whatsapp || '';
  let msg = `Olá! Gostaria de finalizar minha compra do carrinho do site do casamento.`;
  if (nome) msg += `\nNome: ${nome}`;
  if (tel) msg += `\nWhatsApp: ${tel}`;
  msg += `\nItens:`;
  cartItems.value.forEach(item => {
    msg += `\n- ${item.title} (${item.quantity}x) - R$ ${formatPrice(item.value * item.quantity)}`;
  });
  msg += `\nValor total: R$ ${formatPrice(store.cartTotal)}`;
  return encodeURIComponent(msg);
});

const whatsappLinkRebeca = computed(() => `https://wa.me/5577988289651?text=${whatsappMsg.value}`);
const whatsappLinkMarcos = computed(() => `https://wa.me/5577988097836?text=${whatsappMsg.value}`);
      ...
      <div class="modal" @click.stop>
        ...
        <div class="success-content">
          ...
          <p v-if="checkoutData.paymentMethod === 'pix'" class="pix-instruction">
            Seu pagamento foi registrado. Assim que recebermos a confirmação do banco, enviaremos uma mensagem para você.
          </p>
          <p v-else-if="checkoutData.paymentMethod === 'link'" class="link-instruction">
            Você será redirecionado para o link de pagamento. Muito obrigado!
          </p>
...
// Não gera mais QR Code, só exibe dados
...
// Se for compra via link e mais de um item/unidade, não deve prosseguir para links
const confirmLinkPayment = () => {
  if (shouldShowWhatsappModal.value) {
    // Não faz nada, pois o modal já mostra os botões do WhatsApp
    return;
  }
  const allSelected = cartItems.value.every((_, i) => selectedLinks.value[i])
  if (allSelected) {
    // Redireciona para todos os links selecionados
    Object.values(selectedLinks.value).forEach(link => {
      if (link) window.open(link, '_blank')
    })
    savePaymentWithLinks()
    closeLinksModal()
    showSuccessModal.value = true
  } else {
    alert('Por favor, selecione um link para cada item')
  }
}
...
// Ao invés de copiar, mostra modal WhatsApp
function showWhatsappModal() {
  // Força o modal de WhatsApp a aparecer
  showLinksModal.value = true;
  // Garante que o shouldShowWhatsappModal será true
  // (já que o botão só aparece quando for permitido)
}
</script>
