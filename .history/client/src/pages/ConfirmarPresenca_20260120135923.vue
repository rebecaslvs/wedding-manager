        ...
        <div v-else class="confirmation-section">
          ...
          <div v-if="currentGuest.status === 'pending'" class="confirmation-options">
            ...
            <div v-if="showAccompaniantsForm" class="accompanists-form">
              ...
              <div class="form-group">
                <label for="adults">Adultos (pagantes):</label>
                <input
          ...
          <div v-else class="confirmation-status">
            <div v-if="currentGuest.status === 'confirmed'" class="success-message">
              ...
              <p>Obrigado por confirmar sua presença! Estamos ansiosos para vê-lo no grande dia.</p>
              <div class="guest-details">
                <p><strong>Acompanhantes:</strong></p>
                <p>Adultos: {{ currentGuest.adultsCount }}</p>
                <p>Crianças: {{ currentGuest.childrenCount }}</p>
              </div>
            </div>
            <div v-else class="decline-message">
              <h3>✗ Presença Não Confirmada</h3>
              <p>Entendemos que você não poderá comparecer. Sentiremos sua falta!</p>
              <p>Se mudar de ideia, entre em contato conosco.</p>
            </div>
          </div>
...
const submitConfirmation = () => {
  if (currentGuest.value) {
    ...
    store.updateGuest(currentGuest.value.id, {
      ...
      // Aqui você pode salvar os nomes dos acompanhantes se desejar expandir o modelo Guest
    })
